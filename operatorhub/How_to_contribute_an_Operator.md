...
To publish your Operator, it must be built as a binary within a container, and that container must be hosted on a publicly accessible [registry](https://github.com/operator-framework/operator-registry). It should be accompanied by some [metadata](https://github.com/operator-framework/community-operators#know-what-to-contribute) that is used for deploying the Operator using the [Operator Lifecycle Manager](https://github.com/operator-framework/operator-lifecycle-manager) in addition to rendering the Operator’s detail page on OperatorHub.io.

![](../images/operator/operator.png)
...
https://github.com/operator-framework/community-operators#know-what-to-contribute
...
│   ├── kafkatopics.kafka.strimzi.io.crd.yaml
│   ├── kafkausers.kafka.strimzi.io.crd.yaml
│   ├── strimzi-cluster-operator.v0.19.0.clusterserviceversion.yaml
│   ├── strimzientityoperator.clusterrole.yaml
│   ├── strimzikafkabroker.clusterrole.yaml
│   └── strimzitopicoperator.clusterrole.yaml
├── ci.yaml
 ...
 * `opm`, which generates and updates registry databases as well as the index images that encapsulate them.  生成并更新注册表数据库以及封装它们的索引图像。
 * `initializer`, which takes as an input a directory of operator manifests and outputs a sqlite database containing the same data for querying.  它以操作员清单目录作为输入，并输出一个包含相同数据以供查询的sqlite数据库。
 * `registry-server`, which takes a sqlite database loaded with manifests, and exposes a gRPC interface to it.  它使用一个加载了清单的sqlite数据库，并向其公开gRPC接口。
 * `configmap-server`, which takes a kubeconfig and a configmap reference, and parses the configmap into the sqlite database before exposing it via the same interface as `registry-server`.  它需要一个kubeconfig和一个configmap引用，并在通过与注册表服务器相同的接口公开该configmap之前，将configmap解析到sqlite数据库中。
